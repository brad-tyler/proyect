...
class Ssp
{
        ...
        for ($i = 0, $ien = count($data); $i < $ien; $i++) {
            ...
            for ($j = 0, $jen = count($columns); $j < $jen; $j++) {
                ...
                if (isset($column['formatter'])) {
                    $row[$column['db']] = ($isJoin) ? $column['formatter']($data[$i][$column['field']], $data[$i]) : $column['formatter']($data[$i][$column['db']], $data[$i]);
                } else {
                    $row[$column['db']] = (($isJoin) ? $data[$i][$columns[$j]['field']] : $data[$i][$columns[$j]['db']]);
                }
            }
    ...
    static function order($request, $columns, $isJoin = false)
    {
        ...
        if (isset($request['order']) && count($request['order'])) {
            ...
            for ($i = 0, $ien = count($request['order']); $i < $ien; $i++) {
                ...
                $column['db'] = preg_replace("#[ \t]as[ \t].+$#i", "", $column['db']);
    ...
    static function simple($request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '')
    {
        ...
        return array(
            ...
            "totalRecords" => intval($recordsFiltered),
        );
    }
